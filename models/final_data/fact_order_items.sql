...
-- STEP 1: Source Data - Combine Order Items with Orders
WITH src AS (
    SELECT
        oi.ORDER_ITEM_ID,
        oi.ORDER_ID,
        ...
        oi.QUANTITY,
        oi.UNIT_PRICE,
        oi.LINE_TOTAL AS ITEM_TOTAL_AMOUNT,
        o.ORDER_STATUS,
...
-- STEP 3: Detect new or changed item records
changes AS (
...
-- STEP 4: Final output (SCD-like logic)
SELECT * FROM changes

{% if is_incremental() %}
UNION ALL
-- Keep old inactive item records
SELECT
    ORDER_ITEM_ID,
    ORDER_ID,
    CUSTOMER_ID,
